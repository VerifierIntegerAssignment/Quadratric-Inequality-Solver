...
app.layout = html.Div(style={'backgroundColor': colors['background'], 'color': colors['text'], 'height':'100vh', 'width':'100%', 'height':'100%', 'top':'0px', 'left':'0px'}, 
	children=[
		html.Div([
			dcc.Input(id='input_ineqs1', placeholder='Please Enter Variable For Which You Want to Solve Equations.', type='text', style={'width': '50%', 'display': 'inline-block'}),
		]),
		html.Div([
			dcc.Input(id='input_ineqs2', placeholder='Please Enter Pre-Condition.', type='text', style={'width': '50%', 'display': 'inline-block'}),
		]),
		html.Div([
			dcc.Input(id='input_ineqs3', placeholder='Please Enter the First Equation.', type='text', style={'width': '50%', 'display': 'inline-block'}),
		]),
		html.Div([
		...
		html.Div([
			#html.H3(id='output_area', style={'color': colors['text'], 'backgroundColor': colors['background']}),
                        html.Div(id='output_area', style={'white-space': 'pre-line'}),
			])
		])
...
@app.callback(
	...
	[State('input_ineqs1', 'value'), State('input_ineqs2', 'value'),State('input_ineqs3', 'value'),State('input_ineqs4', 'value')]
	)
def update_output(n_clicks, equat1, equat2, equat3, equat4):
	if n_clicks:
      
                return findBound(equat1, equat2, equat3, equat4)
